...
if __name__ == '__main__':
    ...
    with open(f'logs/reb/2modes.estnoi.nonoise.{setting}.{args.dataset}.{args.name}'
              f'.{args.space}.{args.device}.txt',
              'a') as \
            wfid:
        ...
        # for device in devices:
            ...
            exp = f'runs/{args.dataset}.{args.name}.train.searched.scratch' \
                  f'.{args.device}.estnoise.opt2.{setting}.{args.space}.{mode}'

            logger.info(f"running command {pres + [exp]}")
            if not args.print:
                subprocess.call(pres + [exp], stderr=wfid)

            exp =  f'runs/{args.dataset}.{args.name}.train.searched.scratch.nonoise.setting0.' \
                   f'{args.space}.plain.blk{n_blk}s1.1.1/',
