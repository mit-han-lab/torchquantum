...
if __name__ == '__main__':
    ...
    with open(f'logs/reb/devices.simulate.{args.device}.{setting}.{args.dataset}.{args.name}'
              f'.{args.space}.txt',
              'a') as \
            wfid:
        ...
        # for device in devices:
            pres = ['python',
                    'examples/eval.py',
                    f'examples/configs/'
                    f'{args.dataset}/{args.name}/eval/{args.device}/noise/opt2/300.yml',
                    '--jobs=5',
                    f'--hub={args.hub}',
                    '--run-dir']

            exp = f'runs/{args.dataset}.{args.name}.train.searched.scratch' \
                  f'.{args.device}.noise.opt2.{setting}.{args.space}.{mode}'
